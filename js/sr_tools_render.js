var render = {
  ...
  mook_for_action: function ($target, data, options) {
    ...
    $mook.find('.information .sprint .value').html(movement.move + ' / ' + movement.sprint_base + ' + (' + movement.sprint_per_hit_increase + ' meter x ' 
    ...
    $mook.find('.information .sprint button').button().click(function () {
        ...
        $mook.find('.information .sprint .result').html(total + ' meters');
      });
  ...
  calc_movement: function (data, augmented_attributes) {
    ...
    for (skill in data.skills) {  
      if (skill == 'Athletics') sprint_dice = data.skills[skill] + sprint_dice;
    }
